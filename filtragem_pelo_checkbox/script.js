...
function change() {
  ...
  function filterResults(filters) {
    var todosElementosDoResultado = document.querySelectorAll(".result div");
    
    var hiddenElems = [];
  
    if (!todosElementosDoResultado || todosElementosDoResultado.length <= 0) {
      return;
    }
  
    for (var i = 0; i < todosElementosDoResultado.length; i++) {
      var el = todosElementosDoResultado[i];
    ...
    for (var i = 0; i < todosElementosDoResultado.length; i++) {
      ...
      todosElementosDoResultado[i].style.display = "block";
    }
